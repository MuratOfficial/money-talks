...
import { FINGUIDE_RISK_TIP, RiskProfileInfo, allocationText, riskProfileByPercentage } from '@/constants/riskProfiles';



export interface TestResult extends RiskProfileInfo {
  score: number;
...
const TestComponent: React.FC<TestComponentProps> = ({ 
  ...
  const showTestResult = () => {
    ...
    const testResult: TestResult = {
      ...
      percentage,
      // Варианты ответа упорядочены от осторожного к рискованному, и correctAnswer —
      // самый рискованный: доля таких ответов и есть готовность к риску.
      ...riskProfileByPercentage(percentage),
    };
        ...
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          ...
          <View className="px-6 py-8">
            <View className="items-center mb-8">
              ...
              <Text className={`${textColor} text-3xl font-bold mb-2 text-center font-['SFProDisplaySemiBold']`}>
                {result.title}
              </Text>
            ...
            <View className={`${cardBgColor} rounded-2xl p-6 mb-6`}>
              <Text className={`${textColor} text-base mb-2 font-['SFProDisplaySemiBold']`}>
                Рекомендуемое распределение
              </Text>
              <AllocationBar protectiveShare={result.protectiveShare} />
              <Text className={`${textBodyColor} text-sm leading-6 mt-3 font-['SFProDisplayRegular']`}>
                {allocationText(result)}
              </Text>
            </View>

            <Text className={`${textSecondaryColor} text-sm italic mb-6 font-['SFProDisplayRegular']`}>
...
/** Полоса «защита / рост» для распределения портфеля. */
export const AllocationBar = ({ protectiveShare }: { protectiveShare: number }) => (
  <View>
    <View className="flex-row h-3 rounded-full overflow-hidden">
      <View style={{ flex: protectiveShare, backgroundColor: '#3B82F6' }} />
