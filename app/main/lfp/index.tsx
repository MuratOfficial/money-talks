  ...
  return (
    <SafeAreaView className="flex-1 bg-black">
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        <TextInput className="bg-neutral-800 rounded p-3 text-white" placeholderTextColor="#888" placeholder="Введите ФИО" />

        <Text className="text-white mt-4 mb-1">Дата рождения</Text>
        <View className="flex-row justify-between">
          {['День', 'Месяц', 'Год'].map((item) => (
            <TextInput key={item} className="bg-neutral-800 rounded p-3 text-white w-[30%]" placeholder={item} placeholderTextColor="#888" />
          ))}
        </View>

        <Text className="text-white mt-4 mb-1">Деятельность</Text>
        <TextInput className="bg-neutral-800 rounded p-3 text-white" placeholderTextColor="#888" placeholder="Введите деятельность" />

        <Text className="text-white mt-4 mb-1">Финансово-зависимые люди</Text>
        <TextInput className="bg-neutral-800 rounded p-3 text-white" placeholderTextColor="#888" placeholder="Введите информацию" />
        ...
        <View className="mt-4 space-y-3">
          {[
            ...
            <View key={item.label} className="flex-row justify-between bg-neutral-900 p-3 rounded">
        ...
        {[
          ...
          <View key={item.label} className="flex-row justify-between bg-neutral-900 p-3 rounded mb-1">
            <Text className="text-white">{item.label}</Text>
            <Text className="text-white font-semibold">{item.value}</Text>
          </View>
        ))}
        ...
        <View className="bg-neutral-900 p-3 rounded mb-2">
        ...
        {[
          ...
          <View key={i} className="bg-neutral-900 p-3 rounded mb-1">
        ...
        {[
          ...
          <View key={idx} className="bg-neutral-900 rounded p-4 mb-2">
